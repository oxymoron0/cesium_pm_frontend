...
export const API_PATHS = {
  ...
  SIMULATION_QUICK_CIVIL_LIST: getApiPath('api/v1/simulation_auto/civil/list'),
  ...
  SENSOR_DATA_PM10_RANKING: (period: 'current' | 'today' | 'month' | 'year' = 'current', limit: number = 7) =>
    getApiPath(`api/v1/sensor-data/stations/pm10-ranking?period=${period}&limit=${limit}`),

  // 취약시설 알림 랭킹 API (PM Backend)
  VULNERABLE_FACILITIES_ALERT_RANKING: (period: 'current' | 'today' | 'month' | 'year' = 'current', limit: number = 7) =>
