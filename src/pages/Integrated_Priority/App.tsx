...
const App = observer(function App(props: AppProps) {
  ...
  useEffect(() => {
    const checkCesiumStatus = () => {
      ...
      if (isQiankun && parentViewer) {
        console.log('[Integrated_Priority] 부모 Cesium Viewer 감지됨');
        setCesiumStatus('ready');
      } else if (!isQiankun) {
        console.log('[Integrated_Priority] 독립 실행 모드 - Cesium Viewer 초기화 대기');
        const waitForViewer = setInterval(() => {
          if (window.cviewer) {
            console.log('[Integrated_Priority] 독립 Cesium Viewer 초기화 완료');
            setCesiumStatus('ready');
        ...
        const timeout = setTimeout(() => {
          if (cesiumStatus === 'loading') {
            console.error('[Integrated_Priority] Cesium Viewer 초기화 타임아웃');
        ...
        console.error('[Integrated_Priority] Qiankun 환경이지만 부모 Viewer를 찾을 수 없음');
        setCesiumStatus('error');
      }
    };
  ...
  const handleServiceChange = useCallback((service: ServiceType) => {
    console.log(`[Integrated_Priority] Switching service from ${currentService} to ${service}`);
