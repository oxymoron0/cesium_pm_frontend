...
const SimulationConfigInfo = observer(function SimulationConfigInfo({ onClose }: SimulationConfigInfoProps) {
  const { simulationDetail, isConfigPopupMinimized } = simulationStore;
  ...
  return (
    <Panel position="right" width="540px" maxHeight="calc(100vh - 160px)" offset={96}>
      <Title
        onClose={onClose}
        onMinimize={() => simulationStore.toggleConfigPopupMinimize()}
      >
        시뮬레이션 설정 정보
      </Title>

      <div style={{ display: isConfigPopupMinimized ? 'none' : 'contents' }}>
        <Spacer height={16} />
